...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, cos, sin):
        ...
        q = q.view(B, T, N_HEAD, -1)
        k = k.view(B, T, N_HEAD, -1)
        v = v.view(B, T, N_HEAD, -1)

        q = apply_rotary_emb(q, cos, sin)
        k = apply_rotary_emb(k, cos, sin)

        # reshape the q,k,v tensors
        q = q.transpose(1,2) # (B, T, N_H, H_DIM) --> (B, N_H, T, H_DIM)
        k = k.transpose(1,2) # (B, T, N_H, H_DIM) --> (B, N_H, T, H_DIM)
        v = v.transpose(1,2) # (B, T, N_H, H_DIM) --> (B, N_H, T, H_DIM)

        # # apply rotary embeddings
        # # cos shape: [1, seq_len, 1, h_dim/2]
        # q = apply_rotary_emb(q, cos, sin)
...
class GPT(nn.Module):
    ...
    def forward(self, inp, labels=None, ignore_index=-1, use_cache = False, kv_cache=None):
        x = self.tok_emb(inp) # (B,T) 
        B, T, C = x.shape

        if use_cache and kv_cache is not None:
            start_pos = kv_cache[0][0].shape[2] # (num_layers, 2(k,v), T)?
        else:
            start_pos = 0
        ...
        for layer_idx in range(N_LAYERS):
            ...
            x, new_layer_kv_cache = self.h[layer_idx](x, self.cos[:, start_pos : start_pos + T, :, :], self.sin[:, start_pos: start_pos + T, :, :], kv_cache=layer_cache)
            if use_cache:
                new_caches.append(new_layer_kv_cache)
...
# without prefill + kv_cache
def dumb_generate(model, start_prompt, top_p=None, top_k=None, num_tokens=50, num_samples = 16):
...
@torch.no_grad()
def smart_generate(model, start_prompt, top_p=None, top_k=None, num_tokens=50, num_samples=16):
    was_training = model.training
    model.eval()
    ...
    # prepare context
    start_tokens = [stoi[c] for c in start_prompt]
    inp = torch.tensor(start_tokens).view(1,-1).expand(num_samples, len(start_tokens)).to(device)

    # PREFILL STEP
    # Feed the entire prompt, get the final states and logits
    logits, kv_cache = model(inp, use_cache=True)

    if top_p is not None: next_token = pick_next_top_p(logits, top_p)
    elif top_k is not None: next_token = pick_next_top_k(logits, top_k)
    else: next_token = pick_next_greedy(logits)
